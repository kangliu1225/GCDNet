...
args.feat_save_path = 'sentiment.pkl'
...
class ReviewDataset(Dataset):
    ...
    def __init__(self, id,aspect, tokenizer):

        self.id = np.array(id).astype(np.int64)
    ...
    def __getitem__(self, idx):
        return self.id[idx],self.docs[idx]
...
def collate_fn(data):
    ...
    for w_list in tokens:
        s = "This is "+' '.join(w_list)+" ."#@ss s = ' '.join(w_list)
        tokens_new.append(s)
    ...
    return torch.Tensor(id), encoding['input_ids'], encoding['attention_mask'] #torch.Tensor(u), torch.Tensor(i), torch.Tensor(r), encoding['input_ids'], encoding['attention_mask']
...
def save_sentence_feat(params):
    ...
    with open(r'X:\workspace\datasets\Amazon\@aspect\d5\sentiment2id.json', 'r') as f: #@ss
        aspect2id = json.load(f)
    train_dataset = ReviewDataset(
        list(aspect2id.values()),
        list(aspect2id.keys()),
        bert_tokenizer
    )
    ...
    for id, input_ids, mask in tqdm(data_loader):
